...
class Game13157(GameSimulatorTesting):
    ...
    def test_turn_1(self):
        # Deck size 40, Alice, Clues 7, Score 0
        self.load_game(r'games\13157.json', position=0, turn=1, botcls=Bot)
        self.send_action()
        card = self.game.deck[self.bot.hand[0]]
        self.assertCountEqual(card.discardColors,
    ...
    def test_turn_6_0(self):
        # Deck size 37, Bob, Clues 6, Score 2
        self.load_game(r'games\13157.json', position=0, turn=6, botcls=Bot)
        card = self.game.deck[self.bot.hand[0]]
        self.assertCountEqual(card.discardColors,
