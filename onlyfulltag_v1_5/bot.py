...
class Bot(bot.Bot):
    ...
    def __init__(self, game, position, name, wait=0, **kwargs):
        ...
        This table is recomputed every turn.
        '''
        self.taggedCount = {c: [0] * 6 for c in colors}
        '''
    ...
    def possibleValuesInHand(self, color):
        ...
        values = []
        for v in range(len(self.game.playedCards[color]) + 1, 6):
            if self.taggedCount[color][v]:
                continue
            if self.locatedCount[color][v]:
                continue
            if self.eyesightCount[color][v] == maxCount[v]:
                continue
            values.append(v)
        return values
    ...
    def possibleColorsInHand(self, value):
        ...
        for c in colors:
            ...
            if self.locatedCount[c][value]:
                continue
            if self.eyesightCount[c][value] == maxCount[value]:
                continue
            colors_.append(c)
        return colors_
        ...
        for c in range(len(self.game.players[player].hand)):
            ...
            if card.playable is True:
                if card.playable:
                    # Play newest
                    playable = 0
                    if card.suit is not None and card.value is not None:
                        playable += 10
    ...
    def playOrderColor(self, player, color, tagged):
        ...
        for v in range(score + 1, 6):
            found = False
            for t in tagged:
                card = self.game.deck[self.game.players[player].hand[t]]
                if card.value == score + 1:
                    found = True
                    order.append(t)
                    tagged_.remove(t)
                    break
            if not found:
                unknownValue = v
                break
        if unknownValue is not None:
            for v in range(unknownValue, 6):
                pass
    ...
    def bestHintForPlayer(self, player):
        ...
        for c in colors:
            ...
            for i in range(len(hand)):
                ...
                if card.suit & c:
                    ...
                    if (card.value is not None
                            and card.value > self.lowestPlayableValue):
                        needClarify += 1
                    else:
                    ...
                    for t in tagged:
                        ...
                        if tcard.suit == card.suit and tcard.rank == card.rank:
                            useless += 1
                    tagged.append(i)
            if not tagged:
                continue
            ...
            # Decision Here
            if numNewTagged >= 1 and toBePlayable >= 1 and useless == 0:
                colorFitness = needClarify * 10 + otherUseful + numProtect * 5
                if numNewTagged == toBePlayable:
                    colorFitness += 50
            if len(self.game.playedCards[c]) == 4:
                if self.game.deck[hand[tagged[-1]]].rank == 5:
                    colorFitness += 50
        ...
        for v in range(1, 6):
                ...
                if card.rank == v:
                    if card.value is None:
                        ...
                        if self.isNowPlayable(card.suit, v):
                            nowPlayable += 1
                    if not self.isUseful(card.suit, v):
                        useless += 1
                    if self.isCluedElsewhere(player, i):
                        useless += 1
                    for t in tagged:
            ...
            if v < self.lowestPlayableValue:
                ...
                baseValue = 15 if v == 5 else 10
                if numNewTagged >= 1 and nowPlayable >= 1 and useless == 0:
                    valueFitness = ((nowPlayable + numNewTagged) * baseValue)
                    if numNewTagged == nowPlayable:
                        valueFitness += 20
    ...
    def pleaseObserveColorHint(self, from_, to, color, card_indices):
        ...
        for i in range(len(self.game.players[to].hand)):
            ...
            if i in card_indices:
                ...
                elif len(card_indices) == 1 and knol.value is None:
                    knol.cluedAsSingle = True
                elif len(card_indices) >= 2 and incluedKnown5 and score == 4:
                    knol.cluedAsSingle = False
                elif (len(card_indices) == 2 and incluedKnown5
                      and not knol.cluedAsDiscard):
                    knol.cluedAsSingle = True
            else:
        ...
        for i in range(len(self.game.players[to].hand)):
            ...
            if i in card_indices:
                ...
                if knol.color is not None:
                    ...
                    knol.setIsWorthless(True)
                if not worthless and discard in card_indices or value == 5:
                    knol.cluedAsDiscard = True
                elif len(card_indices) == 1 and knol.color is None:
                    knol.cluedAsSingle = True
            else:
