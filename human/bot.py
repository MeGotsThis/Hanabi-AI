...
from variant import sixth_suit
...
class Bot(bot.Bot):
    ...
    def decide_move(self, can_clue, can_discard):
        colorOrder = [BLUE, GREEN, YELLOW, RED, PURPLE]
        sixth = sixth_suit(self.game.variant)
        if sixth is not None and sixth != RAINBOW:
            colorOrder.append(sixth)
            ...
            if i == 0:
                ...
                try:
                    ...
                    for i, c in enumerate(colorOrder):
                        print('({}) {}'.format(i, str_color(c)))
                    i = int(input('--> '))
                    if i < 0 or i >= len(colorOrder):
                        raise ValueError()
                except ValueError:
                    continue
                if self.can_color_clue(w, colorOrder[i]):
                    self.give_color_clue(w, colorOrder[i])
                    break
                else:
                ...
                for i in range(len(self.hand)):
                    ...
                    if card.positiveClueColors or card.positiveClueValue:
                        clues = [str_color(c) for c in card.positiveClueColors]
                    ...
                    if card.negativeClueColors or card.negativeClueValue:
                        clues = [str_color(c) for c in card.negativeClueColors]
                ...
                for c in colorOrder:
                    if self.game.playedCards[c]:
                        ...
                        topCard = 'Empty'
                    print('{}: {}'.format(str_color(c), topCard))
            elif i == 8:
                cards = {
                    BLACK: [],
                    BLUE: [],
                ...
                for co in colorOrder:
                    print('{}: {}'.format(
                        str_color(co), ', '.join(str(c) for c in cards[co])))
            elif i == 9:
